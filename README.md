...
**Linux / macOS:**
```bash
curl -fsSL https://raw.githubusercontent.com/LuaMastery/LunaSpeech/v0.11.0/scripts/install.sh | bash
```

**Windows** (PowerShell):
```powershell
irm https://raw.githubusercontent.com/LuaMastery/LunaSpeech/v0.11.0/scripts/install.ps1 | iex
```
...
> Release **v0.11.0**: <https://github.com/LuaMastery/LunaSpeech/releases/tag/v0.11.0>

## 🌍 Publicar online (site público pra todo mundo)

O LunaSpeech pode virar um **site público** onde qualquer pessoa testa no navegador — basta publicar o servidor (`lunaspeech serve`) num host gratuito. O repositório já vem com `Dockerfile` e `render.yaml`.

**1 clique no Render (grátis):**

[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/LuaMastery/LunaSpeech)

> Após o deploy você recebe um link público (ex.: `https://lunaspeech.onrender.com`) com o **site + API**. O plano free do Render "adormece" após inatividade (o 1º acesso demora ~1 min pra acordar).

**Outras plataformas (Docker):** Hugging Face Spaces (porta 7860), Koyeb, Fly.io.
```bash
docker build -t lunaspeech .
docker run -p 8000:8000 -e PORT=8000 lunaspeech
```
**API pública** (depois de publicado): qualquer site/app chama `https://seu-link/speak?text=olá` → WAV.
